...
{     
        ...
        if (n <limit) {
               jQuery("#profile_location").append('<div id="LengthLoc"><div class="has-btn"><label for="address" class="fw-nrml main-sub-label">Street Address</label><input id="streetaddress" name="streetaddress[]" type="text"><label for="city" class="fw-nrml main-sub-label">City<input type="text" id="city" name="city[]"></label><label for="state" class="fw-nrml main-sub-label">State / Province / Region<input type="text" id="state" name="state[]"></label><label for="zip" class="fw-nrml main-sub-label">ZIP / Postal Code<input type="text" id="postalcode" name="postalcode[]"></label><label for="country" class="fw-nrml main-sub-label">Country<input type="text" id="country" name="country[]"></label><a class="remove-field-btn" alt="remove-company" id="delete">Remove Address</a></div></div>');
        }
        else{
                alert("No more additional fields are allowed!");
        }
...
//Get Level 3 Category
    ...
    $('#profile_type input:radio[name="profile_type"]').change(function(){
       ...
       if(checkprofile==1) { //If profile type is company
        $('#affiliated').removeAttr( "data-validation" );
        ...
        $('#affiliated').attr( "data-validation", "required" );
